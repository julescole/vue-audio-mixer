<template>
    <div
        ...
        :class="{ 'with-panner': mixerVars.show_pan }"
    >
        <div
            class="vue-audio-mixer-channel-panner-container"
            :class="{ 'vue-audio-mixer-is-master': isMaster }"
        >
            <VueKnobControl
        ...
        <div class="logo" v-if="isMaster && !showMute"></div>
        ...
        <div class="vue-audio-mixer-channel-label">
            <label data-label="0"> {{ title }}</label>
        </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        meterWidth() {
            return parseInt(
                variables["meterWidth" + this.mixerVars.theme_size]
            );
        },

        meterWidthBetween() {
            return parseInt(
                variables["meterWidthBetween" + this.mixerVars.theme_size]
            );
        },
    ...
    methods: {
        ...
        titleChange() {
            this.$emit(
                this.mixerVars.instance_id + "titleChange",
                this.titleModel
            );
        },
        ...
        clearCanvas() {
            ...
            this.ctx.fillRect(
                this.meterWidth + this.meterWidthBetween,
                0,
                this.meterWidth,
        ...
        drawMeter() {
            ...
            if (average > this.leftBouncer.average) {
                ...
                if (this.leftBouncer.opacity > 0.1)
                    // fade out
                    this.leftBouncer.opacity = this.leftBouncer.opacity - 0.1;
                else this.leftBouncer.opacity = 0;
            ...
            this.ctx.fillRect(
                0,
                this.meterHeight - average * (this.meterHeight / 100),
                this.meterWidth,
                this.meterHeight + 200
            );
            this.ctx.fillRect(
                this.meterWidth + this.meterWidthBetween,
                this.meterHeight - average2 * (this.meterHeight / 100),
                this.meterWidth,
                this.meterHeight + 200
            );
            ...
            if (average > 0)
                this.ctx.fillRect(
                    0,
                    this.meterHeight -
                        this.leftBouncer.average * (this.meterHeight / 100) -
                        2,
                    this.meterWidth,
                    this.leftBouncer.opacity
                );
            if (average2 > 0)
                this.ctx.fillRect(
                    this.meterWidth + this.meterWidthBetween,
