...
export default {
    ...
    created() {
        ...
        EventBus.$on(
            this.mixerVars.instance_id + "soloChange",
            this.detectedSoloChange
        );
    ...
    computed: {
        ...
        themeClassSize() {
            let className =
                "vue-audio-mixer-theme-" + this.themeSize.toLowerCase();
            let toReturn = {};
        ...
        loading() {
            return (
                this.tracksLoaded == 0 || this.tracksLoaded < this.tracks.length
            );
        },
